...
const dialogCloseButton = dialog.querySelector('span');

for (const pic of picArray) {
    const article = document.createElement('article');
    const header = document.createElement('h2');
    header.textContent = pic.title;
    ...
    article.addEventListener('click', function () {
        showModal(pic.image.large, pic.title);
    });
}

dialogCloseButton.addEventListener('click', close);

function showModal(largeImageSrc, altText) {
  dialogImage.src = largeImageSrc;
